...
const SpinWheel: React.FC<SpinWheelProps> = ({ segments, isSpinning }) => {
  ...
  if (segments.length === 0) {
    return (
      <div 
        className="rounded-full border-4 border-gray-300 border-dashed bg-gray-50 flex items-center justify-center"
        style={{ width: wheelSize, height: wheelSize }}
      >
        <div className="text-center">
          <div className="text-4xl mb-2">🎡</div>
          <p className="text-gray-500 font-medium">Add segments to create your wheel</p>
        </div>
      </div>
    );
  }
              ...
              <g key={segment.id}>
                ...
                {segment.text && (
                  <text
                    ...
                    style={{ 
                      fontSize: Math.min(14, anglePerSegment / 8 + 8),
